...
var calc_average_degree = function (graph) {
    ...
    for (var key in graph) {
        ...
        average += Array.from(neighbors).filter(function (n) { return n; }).length;
    }
    average /= (function () {
        var result = [];
        for (k in graph) {
...
var scoring = {
    ...
    most_guessable_match_sequence: function (password, matches, _exclude_additive) {
        ...
        for (var _b = 0, _c = Array.from(matches_by_j); _b < _c.length; _b++) {
            var lst = _c[_b];
        ...
        var make_bruteforce_match = function (i, j) {
            return {
                pattern: 'bruteforce',
        ...
        for (k = 0, end4 = n, asc4 = 0 <= end4; asc4 ? k < end4 : k > end4; asc4 ? k++ : k--) {
            for (var _d = 0, _e = Array.from(matches_by_j[k]); _d < _e.length; _d++) {
                m = _e[_d];
                if (m.i > 0) {
    ...
    KEYBOARD_AVERAGE_DEGREE: calc_average_degree(adjacency_graphs_1["default"].qwerty),
    // slightly different for keypad/mac keypad, but close enough
    KEYPAD_AVERAGE_DEGREE: calc_average_degree(adjacency_graphs_1["default"].keypad),
    KEYBOARD_STARTING_POSITIONS: (function () {
        var result = [];
        for (k in adjacency_graphs_1["default"].qwerty) {
            v = adjacency_graphs_1["default"].qwerty[k];
            result.push(k);
        }
        return result;
    })().length,
    KEYPAD_STARTING_POSITIONS: (function () {
        var result1 = [];
        for (k in adjacency_graphs_1["default"].keypad) {
            v = adjacency_graphs_1["default"].keypad[k];
            result1.push(k);
        }
        return result1;
    })().length,
    ...
    uppercase_variations: function (match) {
        ...
        for (var _i = 0, _a = [this.START_UPPER, this.END_UPPER, this.ALL_UPPER]; _i < _a.length; _i++) {
            var regex = _a[_i];
            if (word.match(regex)) {
                return 2;
            }
        ...
        var U = (function () {
            var result2 = [];
            for (var _i = 0, _a = Array.from(word.split('')); _i < _a.length; _i++) {
                chr = _a[_i];
                if (chr.match(/[A-Z]/)) {
                    result2.push(chr);
                }
    ...
    l33t_variations: function (match) {
        ...
        for (var subbed in match.sub) {
            ...
            var S = (function () {
                var result2 = [];
                for (var _i = 0, _a = Array.from(chrs); _i < _a.length; _i++) {
                    chr = _a[_i];
                    if (chr === subbed) {
                        result2.push(chr);
                    }
